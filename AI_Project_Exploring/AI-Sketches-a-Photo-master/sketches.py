...
# This program generates a light pencil sketch of a photo (drawing.png),
# And a more accentuated, thick penciled detailed sketch of a photo (final.png)
# The outline of the major edges of the photo can be seen in edges.png

filename = "test_X5M.jpeg"
# filename = "test_Ballon.jpeg"
...
class Drawing:
    ...
    def drawing(self):
        ...
        # Follow the process to apply the blur to the image, and to reverse the colors
        # so it's white outline on black background
        # Change to black and white

        # Convert between RGB/BGR and grayscale, color conversions
        # RGB color is model contains Red, Green, Blue
        ...
        # 13 is the Blury Level, hgiher the number, more blury it would be. Range: 1 to 25
        blur = cv.GaussianBlur(inverse, (13,13), 0)
        
        # After applying the blur, undo the inversion by dividing the matrices on a 256 scale
        # Need research on this one, why its dividing
...
# Threshold is set to each pixel in the drawing. If the pixel value is set below the threshold (240) assigned 0.
# 255 is the max which is assigned to pixel values
...
git remote add origin https://github.com/StartSlowButGrowFast/Pencil-Sketch-Converter.git
git branch -M main
